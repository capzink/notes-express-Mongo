...
const Note = require('../models/mdnote'); //helper


const {
  isAuthenticated
} = require('../helpers/auth'); //pagina de formulario para nueva nota
...
router.post('/notes/newnote', isAuthenticated, async (req, res) => {
  ...
  if (errors.length > 0) {
    res.render('notes/newnote', isAuthenticated, {
      errors,
      title,
      description
    });
